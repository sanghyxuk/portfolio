      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projectData.map((project) => (
            <Link to={`/project/${project.id}`} key={project.id} className="bg-white rounded-2xl border border-gray-200 hover:-translate-y-2 hover:shadow-2xl hover:border-gray-300 transition-all duration-500 group flex flex-col overflow-hidden cursor-pointer">
              
              {/* 📸 카드는 무조건 대표 사진(image)만 보여줍니다 */}
              {project.image ? (
