  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200">
      <div className="max-w-6xl mx-auto px-6 h-16 flex justify-between items-center">
        <a href="#" className="font-extrabold text-2xl text-black tracking-tighter">
          Sanghyuk.log
        </a>
          ...
          >
            ...
            <div className={`absolute top-full right-0 pt-2 w-56 transition-all duration-200 ${isDropdownOpen ? 'opacity-100 translate-y-0 visible' : 'opacity-0 translate-y-2 invisible'}`}>
              <div className="bg-white border border-gray-200 shadow-xl rounded-xl overflow-hidden p-2">
                {projectData.map(p => (
                  <a key={p.id} href={`#${p.id}`} className="block px-4 py-2.5 hover:bg-gray-100 rounded-lg text-sm text-gray-700 hover:text-black transition-colors truncate">
                    {p.title}
                  </a>
                ))}
              </div>
